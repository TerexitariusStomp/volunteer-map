#!/usr/bin/env python3
import urllib.request, json, re, os, time, sqlite3, sys
...
LOG_FILE = '/root/workspace/integration/enrichment.log'
...
def find_external_website(text, base_url):
    ...
    blocked_domains = [
        ...
        # Spellcheck / browser
        'spellcheck.net', 'browserstack.com', 'sentry.io',
    ]
    ...
    for u in urls:
        try:
            ...
            if len(parsed.path) < 3 and parsed.query:
                # Likely just a tracking pixel endpoint
                continue
...
# Checkpoint
start_idx = 0
cp_file = f'{CHECKPOINT_DIR}/enrich_progress.json'
